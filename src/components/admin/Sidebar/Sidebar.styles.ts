...
const navLinkSharedStyles = css<{ $isActive?: boolean; theme: DefaultTheme }>`
  ...
  color: ${(props) =>
    props.$isActive
      ...
      : props.theme.colors.adminText};
  text-decoration: none;
  transition: all 0.2s ease-out;
  background-color: ${(props) =>
    props.$isActive ? rgba(props.theme.colors.accent1, 0.08) : "transparent"};
  cursor: pointer;
...
export const NavLinkStyled = styled(Link).withConfig({
  shouldForwardProp: (prop) =>
    !["$isActive", "$isSubItem", "$hasSubItems"].includes(prop),
})<NavLinkProps>`
  ${navLinkSharedStyles}
  ...
  ${(props) =>
    ...
    css`
      ...
      color: ${props.$isActive
        ? props.theme.colors.accent1
        : props.theme.colors.adminTextSecondary};
...
export const SubNavList = styled.ul<{ $isOpen?: boolean }>`
  ...
  background-color: ${(props) =>
    rgba(props.theme.colors.adminSecondaryBg, 0.5)};
