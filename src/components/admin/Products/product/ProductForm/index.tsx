...
import { mapProductResponseToFormState } from "@/utils/productFormUtils";
...
const ProductForm: React.FC = () => {
  ...
  const { data: categoriesData, isLoading: isLoadingCategories } =
    useGetPaginatedCategories(
      { limit: 1000, sort: JSON.stringify({ name: 1 }) },
  ...
  const handleMutationError = useCallback(
    (error: any) => {
      ...
      if (typeof error === "string") {
        ...
        displayMessage = error.errors.join(" ");
      }
  ...
  const createProductMutation = useCreateProduct({
    onSuccess: (res) => {
      showNotification(
        `Product "${res.data.name}" created successfully!`,
        "success"
      );
  ...
  const updateProductMutation = useUpdateProduct({
    onSuccess: (res) => {
      showNotification(
        `Product "${res.data.name}" updated successfully!`,
        "success"
      );
      navigate("/admin/products");
    },
  ...
  useEffect(() => {
    if (!isEditMode) setFormData(initialProductFormState);
  }, [isEditMode]);
  ...
  const handleMetaKeywordsChange = useCallback((keywords: string[]) => {
    setFormData((prev) => ({ ...prev, metaKeywords: keywords }));
  }, []);
  const handleImagesUpdate = useCallback(
    (files: File[], publicIdsToDelete: string[]) => {
      setFormData((prev) => ({
        ...prev,
  ...
  const handleSubmit = (e: FormEvent) => {
    ...
    if (!formData.categoryId)
      validationErrors.categoryId = "A category must be selected.";
    if (formData.variations.length === 0) {
      showNotification(
        "A product must have at least one variation.",
        "warning"
      );
      return;
    }
    ...
    if (isEditMode && productId)
      updateProductMutation.mutate({ productId, formData: apiFormData });
    else createProductMutation.mutate(apiFormData);
  };

  const isMutating =
    createProductMutation.isPending || updateProductMutation.isPending;
  const isLoading = isEditMode || isLoadingCategories;

  if (isLoading)
    return (
      <LoadingSpinner
        message={isEditMode ? "Loading form..." : "Loading form..."}
        fullscreen
      />
    );
  ...
  return (
    <ProductFormContainer>
      ...
      <ActualProductForm onSubmit={handleSubmit} noValidate>
        ...
        {/* ============================================================== */}
        {/* === CORRECTED AND COMPLETE PRODUCT IMAGES SECTION ============ */}
        {/* ============================================================== */}
