...
import {
  useGetVendorApplication as useGetVendorApplicationById,
  useUpdateVendorProfileStatusById as useUpdateVendorApplicationStatusById,
} from "@/hooks/admin/application/useVendor";

import { useNotification } from "@/contexts/NotificationContext";

import {
  getValidNextStatuses as getValidNextVendorStatuses,
  isRejectionReasonRequired as isVendorRejectionReasonRequired,
...
const FallbackLoadingOverlay = styled.div`
  ...
  .spinner-icon {
    font-size: 2.5rem;
    color: ${(props) => props.theme.colors.accent1 || "blue"};
    margin-bottom: 1rem;
...
const FallbackErrorMessage = styled.div`
  padding: 20px;
  text-align: center;
  color: ${(props) => props.theme.colors.adminStatusError || "red"};
  border: 1px solid ${(props) => props.theme.colors.adminStatusError || "red"};
  margin: 20px;
  background-color: ${(props) =>
    props.theme.colors.adminErrorBg || "rgba(255,0,0,0.05)"};
  border-radius: ${(props) => props.theme.borderRadius.medium || "8px"};
  button {
    margin-top: 10px;
  }
...
type ApplicationActionForHeader =
  | "approve"
  | "reject"
  | "suspend"
  | "activate"
...
const VendorApplicationDetail: React.FC<VendorApplicationDetailProps> = ({
  ...
  const onApplicationAction = (
    ...
    showConfirmModal(
      ...
      () => {
        console.log(
          `CONFIRMED ${action.toUpperCase()} for ${appType} application ID: ${appId}`
        );
  ...
  const validStatusOptions = useMemo(() => {
    ...
    const options = nextStatuses.map((status) => ({
      value: status,
      label:
        ALL_VENDOR_STATUS_LABELS[status] ||
        status.charAt(0).toUpperCase() + status.slice(1),
    }));
    return [
      { value: "", label: "Select a New Status...", disabled: true },
      ...options,
    ];
  }, [application?.status]);
  ...
  const handleUpdateStatusFromDropdown = useCallback(async () => {
    ...
    if (
      ...
      (newStatus !== "rejected" ||
        rejectionReason === (application as any).rejectionReason)
    ) {
    ...
    if (
      newStatus !== application.status &&
      !getValidNextVendorStatuses(application.status).includes(
        newStatus as VendorApplicationStatus
      )
    ) {
      showNotification(
        `Transition from '${application.status}' to '${newStatus}' is not allowed.`,
        "error"
      );
      return;
    }
    if (
      isVendorRejectionReasonRequired(newStatus as VendorApplicationStatus) &&
      !rejectionReason.trim()
    ) {
      showNotification(
        "Rejection reason is required when changing status to 'Rejected'.",
        "error"
      );
      return;
    }
    ...
    updateStatusMutation(
      { applicationId: application._id, ...payload }, // Variables for mutation hook
      {
        onSuccess: (response) => {
          showNotification(
            `Application status successfully updated to ${
  ...
  const formatBoolean = (value?: boolean): React.ReactNode => {
    ...
    return value ? (
      <span
        ...
        style={{ color: theme.colors.adminStatusSuccess || "green" }}
      >
        Yes
      </span>
    ) : (
      <span
        ...
        style={{ color: theme.colors.adminStatusError || "red" }}
      >
        No
      </span>
    );
  ...
  if (!applicationId) {
    return (
      <DetailContainer>
        ...
        <InfoBox>
          <p>
            No application ID provided. Please select an application from the
            list.
          </p>
        </InfoBox>
      </DetailContainer>
    ...
    return (
      <DetailContainer>
        <AdminButton
          $variant="secondary"
          onClick={onBackToList}
          style={{ alignSelf: "flex-start", marginBottom: theme.spacing(4) }}
        >
          <FaArrowLeft style={{ marginRight: theme.spacing(1) }} /> Back
        </AdminButton>
        <FallbackLoadingOverlay>
          <FaSpinner className="spinner-icon" />{" "}
          <p>Loading Vendor Application...</p>
        </FallbackLoadingOverlay>
      </DetailContainer>
    );
  }
    ...
    return (
      <DetailContainer>
        ...
        <FallbackErrorMessage>
          <p>
            {fetchError?.message ||
              `Vendor Application with ID "${applicationId}" not found or failed to load.`}
          </p>
          <AdminButton $variant="neutral" onClick={() => refetch()}>
            Try Again
          </AdminButton>
        </FallbackErrorMessage>
      </DetailContainer>
  ...
  const businessDocumentDisplay = doc
    ? typeof doc === "string"
      ? doc
      : doc.name
    : "N/A";
  ...
  return (
    <DetailContainer>
      ...
      <DetailHeader>
        <HeaderInfo>
          <h2>
            <FaBuilding style={{ marginRight: theme.spacing(2) }} />{" "}
          ...
          <span className="applicant-id">
            ...
            {typeof application.userId === "string"
              ? application.userId
              : (application.userId as any)?._id || "N/A"}
          </span>
        </HeaderInfo>
        <HeaderActions>
          {validNextForHeader.includes("approved") &&
            (currentAppStatus === "pending" ||
              currentAppStatus === "submitted" ||
              currentAppStatus === "processing") && (
              <AdminButton
                $variant="success"
                onClick={() =>
                  onApplicationAction(
              ...
              >
                <FaCheckCircle style={{ marginRight: theme.spacing(1) }} />{" "}
                Approve App
              </AdminButton>
            )}
          {validNextForHeader.includes("rejected") &&
            (currentAppStatus === "pending" ||
              ...
              <AdminButton
                ...
                onClick={() => {
                  const reason = prompt(
                    "Reason for rejection (required for header action):"
                  );
                  if (reason === null) return;
                  if (
                    !reason.trim() &&
                    isVendorRejectionReasonRequired("rejected")
                  ) {
              ...
              >
                <FaTimesCircle style={{ marginRight: theme.spacing(1) }} />{" "}
                Reject App
              </AdminButton>
            )}
          {validNextForHeader.includes("active") &&
          ...
          {validNextForHeader.includes("suspended") &&
            currentAppStatus === "active" && (
              ...
              >
                <FaHourglassHalf style={{ marginRight: theme.spacing(1) }} />{" "}
                Suspend Vendor
              </AdminButton>
            )}
          {validNextForHeader.includes("inactive") &&
            currentAppStatus === "active" && (
              <AdminButton
                $variant="danger"
                onClick={() =>
      ...
      <DetailLayout>
        <MainContentColumn>
          <InfoBox>
            ...
            <InfoGrid>
              <InfoItem>
                <label>Company Name</label> <p>{application.companyName}</p>
              </InfoItem>
              <InfoItem>
                <label>Business Reg. No.</label>{" "}
                <p>{application.businessRegistrationNumber || "N/A"}</p>
              </InfoItem>
              <InfoItem>
                <label>Legal Entity Type</label>{" "}
                <p>{application.legalEntityType?.toUpperCase() || "N/A"}</p>
              </InfoItem>
              <InfoItem>
                <label>Year Established</label>{" "}
                <p>{application.yearEstablished || "N/A"}</p>
              </InfoItem>
              <InfoItem $fullWidth>
                <label>Website</label>{" "}
                {application.website ? (
                  ...
                  >
                    <FaGlobe style={{ marginRight: theme.spacing(1) }} />{" "}
                    {application.website}
                  </a>
                ) : (
                  <p>N/A</p>
              ...
              <InfoItem $fullWidth>
                ...
                <p>
                  <FaMapMarkerAlt style={{ marginRight: theme.spacing(1) }} />{" "}
                  {renderAddress(application.companyAddress)}
                </p>
              </InfoItem>
            </InfoGrid>
          </InfoBox>
          ...
          <InfoBox>
            ...
            <InfoGrid>
              <InfoItem>
                ...
                <p>
                  {application.contactPersonFirstName}{" "}
                  {application.contactPersonLastName}
                </p>
              </InfoItem>
              <InfoItem>
                <label>Email</label>{" "}
                <p>
                  <FaEnvelope style={{ marginRight: theme.spacing(1) }} />{" "}
              ...
              <InfoItem>
                <label>Phone</label>{" "}
                <p>
                  <FaPhone style={{ marginRight: theme.spacing(1) }} />{" "}
                  {application.contactPersonPhone || "N/A"}
                </p>
              </InfoItem>
              <InfoItem>
                <label>Role</label>{" "}
          ...
          <InfoBox>
            <InfoSectionTitle>
              <FaBriefcase /> Business Details & Categories
            </InfoSectionTitle>
            <InfoGrid>
              <InfoItem>
                <label>Primary Categories</label>{" "}
                <p>
                  {application.primaryProductCategories?.join(", ") || "N/A"}
                </p>
              </InfoItem>
              <InfoItem>
                <label>Est. Monthly Sales</label>{" "}
                <p>
                  <FaDollarSign style={{ marginRight: theme.spacing(1) }} />{" "}
                  {application.estimatedMonthlySales?.toLocaleString() || "N/A"}
                </p>
              </InfoItem>
              <InfoItem $fullWidth>
                ...
                {doc ? (
                  <DocumentLink
                    ...
                    rel="noopener noreferrer"
                  >
                    {typeof doc !== "string" && doc.type?.includes("image") ? (
                      <img
                        src={doc.url}
                        alt={doc.name}
                        style={{
        ...
        <SidebarContentColumn>
          <InfoBox>
            <InfoSectionTitle>
              <FaIdCard /> Financial & Tax Information
            </InfoSectionTitle>
            <InfoGrid style={{ gridTemplateColumns: "1fr" }}>
              <InfoItem>
                <label>Company Tax ID</label>{" "}
                <p>{application.companyTaxId || "N/A"}</p>
              </InfoItem>
              <InfoItem>
                <label>Business Bank Name</label>{" "}
                <p>{application.businessBankName || "N/A"}</p>
              </InfoItem>
              <InfoItem>
                <label>Business Acct No.</label>{" "}
                <p>
                  ****
                  {application.businessBankAccountNumber?.slice(-4) || "****"}
                </p>
              </InfoItem>
            </InfoGrid>
          </InfoBox>
                ...
                {application.members.map((member, index) => (
                  <MemberItem key={(member.userId || "member") + index}>
                    ...
                    <span className="member-roles">
                      {member.roles?.join(", ").replace(/_/g, " ") ||
                        "No roles"}
                    </span>
                    {member.addedAt && (
                      <span className="added-info">
                        Added: {formatDate(member.addedAt)} by{" "}
          ...
          <InfoBox>
            <InfoSectionTitle>
              <FaShieldAlt /> Agreements
            </InfoSectionTitle>
            <InfoGrid style={{ gridTemplateColumns: "1fr" }}>
              <InfoItem>
                <label>Agreed to Terms</label>{" "}
                {formatBoolean(application.agreedToTerms)}
              </InfoItem>
              <InfoItem>
                <label>Agreed to Privacy Policy</label>{" "}
                {formatBoolean(application.agreedToPrivacyPolicy)}
              </InfoItem>
            </InfoGrid>
          </InfoBox>
          ...
          <InfoBox>
            ...
            <CurrentStatusDisplay>
              ...
              <VendorApplicationStatusBadge
                $status={application.status as VendorApplicationStatusType}
              >
                {ALL_VENDOR_STATUS_LABELS[
                  application.status as VendorApplicationStatusType
            ...
            <InfoGrid style={{ gridTemplateColumns: "1fr" }}>
              <InfoItem>
                ...
                <p>
                  <FaCalendarAlt style={{ marginRight: theme.spacing(1) }} />{" "}
              ...
              {application.approvedBy && (
                <InfoItem>
                  ...
                  <p>
                    {typeof application.approvedBy === "string"
                      ? application.approvedBy
                      : (application.approvedBy as any)?.name || "System"}
                  </p>
                </InfoItem>
              )}
            ...
            <ActionPanel>
              <FormLabel htmlFor="statusChangeSelectVendor">
                Change Status To:
              </FormLabel>
              <AdminSelect
              ...
              {newStatus === "rejected" && (
                <div style={{ marginTop: theme.spacing(3), width: "100%" }}>
                  <FormLabel htmlFor="rejectionReasonInputVendor">
              ...
              <AdminButton
                ...
                disabled={
                  ...
                  newStatus === application.status ||
                  isUpdatingStatus ||
                  (isVendorRejectionReasonRequired(
                    newStatus as VendorApplicationStatus
                  ) &&
                    !rejectionReason.trim())
                }
