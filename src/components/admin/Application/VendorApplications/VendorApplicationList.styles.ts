...
// const getTheme = (props: { theme: DefaultTheme }) => props.theme; // Direct access to props.theme
...
export const VendorApplicationListContainer = styled.div`
    display: flex;
...
export const VendorApplicationListHeader = styled.div`
    display: flex;
...
export const VendorApplicationSearchInput = styled.input`
    ...
    color: ${props => props.theme.colors.adminText};
    width: 350px; /* Can be adjusted for vendor specific searches */
...
export const AdminTableWrapper = styled.div`
    ...
    &::-webkit-scrollbar-track { background: ${(props) => props.theme.colors.adminSecondaryBg}; border-radius: 10px; }
...
export const AdminTable = styled.table`
    ...
    border-spacing: 0;
    min-width: 950px; /* Adjusted for vendor columns */
    ...
    thead th {
        ...
        &:hover { color: ${(props) => props.theme.colors.adminText}; }
    ...
    tbody tr {
        ...
        &:hover { background-color: ${(props) => rgba(props.theme.colors.accent1, 0.03)}; }
    }
...
export const TableActionButton = styled.button`
    ...
    &:hover { color: ${(props) => props.theme.colors.accent1}; }
    &:not(:last-child) { margin-right: ${(props) => props.theme.spacing(2)}; }
`;

// Specific status badge for Vendor Applications if statuses are different or require different styling
// For now, assuming VendorProfileStatus aligns enough with general application statuses.
// If VendorProfileStatus has unique values like 'active', 'inactive', 'closed', 'withdrawn'
// that need distinct styling, we should create a new VendorApplicationStatusBadge component.
// For now, let's use a general one or adapt ApplicationStatusBadge from Seller's styles.
// Let's define it here with vendor specific statuses for clarity
export type VendorApplicationStatusType = "pending" | "approved" | "rejected" | "active" | "inactive" | "suspended" | "closed" | "withdrawn";

export const VendorApplicationStatusBadge = styled.span<{ $status: VendorApplicationStatusType }>`
    padding: 4px 8px;
    border-radius: 6px;
    ...
    ${(props) => props.$status === 'suspended' && css`
        background-color: ${rgba(props.theme.colors.adminTextSecondary, 0.25)}; /* Darker suspended */
...
export const PaginationButton = styled.button<{ $active?: boolean }>`
    ...
    &.prev-next-btn { padding: ${(props) => props.theme.spacing(1)} ${(props) => props.theme.spacing(3)}; }
`;
